...
def build_shifts_data(all_rows, mapping):
    mapping_by_name = {item["name"].lower(): item for item in mapping}
    name_to_row = {r["name"].lower(): r for r in all_rows}

    ht_names = [n.lower() for n in SHIFTS_HIGH_TICKET_SEEDS]
    bulk_names = [n.lower() for n in SHIFTS_BULK_SEEDS]

    ht_rows   = [name_to_row[n] for n in ht_names   if n in name_to_row]
    bulk_rows = [name_to_row[n] for n in bulk_names if n in name_to_row]
